...
You need to import `percept-ui.css` file from `percept-ui` package in order to use applied css styles in components

```bash copy
import 'percept-ui/percept-ui.css'
```
or ` You can install TailwindCSS` Refer to [TailwindCSS](https://tailwindcss.com/docs/installation) Documentation
